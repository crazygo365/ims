...
export const ShopForm: React.FC<ShopFormProps> = ({ onSubmit, initialData, isEdit }) => {
  ...
  const handleInvestorChange = (index: number, field: 'name' | 'investment' | 'shares', value: string) => {
    const newInvestors = [...investors];
    if (field === 'investment' || field === 'shares') {
      newInvestors[index][field] = Number(value) || 0;
    } else {
                ...
                <tr key={investor.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input
                      ...
                      className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-right"
                      required
                    />
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input
                      ...
                      onChange={(e) => handleInvestorChange(index, 'shares', e.target.value)}
                      className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-right"
                      required
                    />
                  </td>
